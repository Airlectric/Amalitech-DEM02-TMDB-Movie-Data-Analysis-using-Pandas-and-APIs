...
def calculate_roi(df):
    df = df.copy()
    df['roi'] = df.apply(lambda x : x['revenue']/ x['budget'] if x['budget'] >= 10 else np.nan, axis=1)
    return df
...
def get_kpis(df):
    ...
    return {
        "higest_revenue": rank_movies(df, "revenue", ascending=False),
        "higest_budget": rank_movies(df, "budget", ascending=False),
